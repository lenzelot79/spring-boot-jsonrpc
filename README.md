...
**Registration and instatiation of the Client**

To get an instance of the client a bean as followos must be registered. For test the instance can be created directly in the test class. 
...
In this example an instance of `MyJsonRpcControllerClient` is generated by te the framework (1). Part (2) is optional. If it is missing the library uses 
the default `RestTemplate` from spring. Use your own RestTemplate if you need to inject some headers like *Authorization*, or if you need a special RestTemplate
like `KeycloakRestTemplate` if you are working with Keycloak. The base URL of your service must be defined as shown in (3).
